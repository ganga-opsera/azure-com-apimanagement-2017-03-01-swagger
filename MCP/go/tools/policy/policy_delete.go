...
func Policy_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePolicy_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_policies_policyId",
		mcp.WithDescription("Deletes the global policy configuration of the Api Management Service."),
		...
		mcp.WithString("If-Match", mcp.Required(), mcp.Description("The entity state (Etag) version of the policy to be deleted. A value of \"*\" can be used for If-Match to unconditionally apply the operation.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Policy_deleteHandler(cfg),
	}
}
